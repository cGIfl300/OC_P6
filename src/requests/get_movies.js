import {maximumMovies, serverURL, typeOfMovies} from "../variables/variables";


let movies = Array();
let actualPage = 1;

export function getMovies(selectedElement, requestedFilter = "best", numberOfMovies) {
...
function requestDB(requestedFilter, numberOfMovies) {
    ...
    xmlhttp.open(method, serverURL + urlRequest + "&page=" + actualPage, false);
...
function stockMovies(rawStock, numberOfMovies, requestedFilter) {
    ...
    for (element;
         (element < rawStock.results.length) && (movies.length < maximumMovies) && (movies.length < numberOfMovies);
         element++) {
        // DEBUG
        console.log("Page (" + actualPage + ") Adding " + rawStock.results[element].title + " at " + rawStock.results[element].imdb_url);
        // END DEBUG
